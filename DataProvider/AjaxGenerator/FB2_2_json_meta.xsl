<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink"
	...
	<xsl:template match="/*">
	<xsl:text>Meta:{Tile:"</xsl:text><xsl:value-of select="fb2js:Escape(/fb:FictionBook/fb:description/fb:title-info/fb:book-title,0)"/>",UUID:"<xsl:value-of select="/fb:FictionBook/fb:description/fb:document-info/fb:id"/>",Authors:[<xsl:for-each select="/fb:FictionBook/fb:description/fb:title-info/fb:author">{First:"<xsl:value-of select="fb2js:Escape(fb:first-name,0)"/>",Last:"<xsl:value-of select="fb2js:Escape(fb:last-name,0)"
