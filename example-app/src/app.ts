...
var AFB3Reader: IFBReader;
var AFB3PPCache: IFB3PPCache;
var BookmarksProcessor: IBookmarks;
var start: number;
var LocalArtID = 11668997;
var Temp = 0;
var LitresLocalBookmarks = new LocalBookmarks.LocalBookmarksClass(LocalArtID.toString());
let AReaderDOM : FB3DOM.DOM;
var AppVersion = '1.2';

let textCacheManager: ITextCacheManager = undefined;
let mediaCacheLoader: FB3MediaCache.MediaCacheLoader = undefined;

const baseUrl = GetBaseURL() || DefaultBaseUrl();
...
window.onload = () => {
	...
	FB3IndexedDBDriver.getDriver().open();
	textCacheManager = FB3TextCache.getTextCacheManager(AppVersion);
	const artKey = textCacheManager.GetStorageKey(baseUrl);
	textCacheManager.Use(() => {
		textCacheManager.InitializeTextCacheData(artKey, LocalArtID, false, 'auto');
		mediaCacheLoader = new FB3MediaCache.MediaCacheLoader(textCacheManager, baseUrl);
		AReaderDOM.MediaCacheLoader = mediaCacheLoader;
	}, () => {});

	var SID = GetSID();
	var ArtID = '123456789';
	var Canvas = document.getElementById('reader');
	var AReaderSite = new FB3ReaderSite.ExampleSite(Canvas, AFB3Reader, LitresLocalBookmarks);
	var DataProvider = new FB3DataProvider.AJAXDataProvider(baseUrl, ArtID2URL, textCacheManager);
	AFB3PPCache = new FB3PPCache.PPCache(AppVersion, StorageType.NoStorage);
	AReaderDOM = new FB3DOM.DOM(AReaderSite, AReaderSite.Progressor, DataProvider, AFB3PPCache);
...
function DefaultBaseUrl(): string {
	return '/DataProvider/AjaxExample/' + LocalArtID + '/';
}

var MarkupProgress: string;
var NativeNote: IBookmark;
var RoundedNote: IBookmark;
...
function MouseMove(Evt: MouseEvent) {
	if (NativeNote && NativeNote.Group == 3 && !MenuShown && !DialogShown) {
		...
		Coords = false;
		var CurrentElPos: IPosition = AFB3Reader.ElementAtXY(CurrCoords.X, CurrCoords.Y);
		if (CurrentElPos && CurrentElPos.length && StartElPos && StartElPos.length) {
			...
			var NewNote: IBookmark = NativeNote.RoundClone(false);
...
function UpdateRange(StartPos: IPosition, EndPos: IPosition) {
...
var DialogBookmark: IBookmark;
function ShowDialog(Bookmark: IBookmark) {
...
function Toc2Div(TOCS: ITOC[]): string {
	var Out = '';
...
function Bookmark2Div(Bookmark: IBookmark): string {
...
function GoXPath(NewPos: IPosition): void {
