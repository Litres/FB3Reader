import {IArtID2URL, IJSonLoadedCallback, IJsonLoaderFactory, RequestConfiguration} from "./FB3DataProvider.head";
import {FB3TextCache} from "../TextCache/TextCache";
import {ILoadProgress} from "../Site/FB3ReaderSite.head";
import {ITextCacheManager} from "../TextCache/TextCache.head";

export module FB3DataProvider {
	...
	export class AJAXDataProvider implements IJsonLoaderFactory {
		...
		constructor(public LitresURL: string, public ArtID2URL: IArtID2URL, private TextCacheManager?: ITextCacheManager) {
			this.BaseURL = LitresURL;
			this.CurrentRequestID = 0;
			this.ActiveRequests = {};
		}
		public Request(URL: string, Callback: IJSonLoadedCallback, Progressor: ILoadProgress, CustomData?: any, IgnoreCache: boolean = false) {
			if (this.TextCacheManager && !IgnoreCache) {
				this.TextCacheManager.Use(() => {
					this.TextCacheManager.LoadChunkData(this.TextCacheManager.GetStorageKey(this.BaseURL), URL, (data, customData) => {
						if (data) {
			...
			if (IgnoreCache === true) {
				URL = FB3TextCache.NoCacheURL(URL);
			}

			this.InitRequest(URL, Callback, Progressor, CustomData);
		}
		private InitRequest(URL: string, Callback: IJSonLoadedCallback, Progressor: ILoadProgress, CustomData?: any) {
		...
		private CallbackWrap(ID:number, URL:string, Data: any, CustomData?: any): void {
			if (this.TextCacheManager) {
				this.TextCacheManager.Use(()=> {
					this.TextCacheManager.SaveChunkData(this.TextCacheManager.GetStorageKey(this.BaseURL), URL, Data, CustomData, () => {
	...
	class AjaxLoader {
		...
		constructor(public URL: string,
			...
			private Progressor: ILoadProgress,
			private ID: number,
 ...
 */
export module AjaxDataProvider {
	/**
	...
	export class AjaxLoader {
		...
		constructor(Config: RequestConfiguration) {
			if (Config.Method) {
