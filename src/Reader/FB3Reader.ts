import {IFBReader, IPageRenderInstruction} from "./FB3Reader.head";
import {FB3ReaderPage} from "./FB3ReaderPage";
import {IFB3PPCache} from "../Storage/Storage.head";
import {IAlert, IFB3ReaderSite} from '../Site/FB3ReaderSite.head'
import {FB3BookReadProgress} from "./ReadProgress";
import {IActiveZone, IFB3Block, IFB3DOM, IPageContainer, IRange, ITOC, IXPath} from "../DOM/FB3DOM.head";
import {ICanvasReadyCallback, IDimensions, IPosition} from "../FB3ReaderHeaders";
import {IBookmarks} from "../Bookmarks/FB3Bookmarks.head";
import {getUnixTime} from "date-fns"

export module FB3Reader {
	...
	export function RangeClone(BaseRange: IRange): IRange {
		return {
	...
	export class Reader implements IFBReader {
		...
		private SetStartPos(NewPos: IPosition): void {
			...
			if (!this.Bookmarks.Bookmarks[0].SkipUpdateDatetime) {
				this.Bookmarks.Bookmarks[0].DateTime = getUnixTime(new Date());
			} else {
		...
		constructor(
			...
			public Site: IFB3ReaderSite,
			public FB3DOM: IFB3DOM,
			...
			private OutOfBoundsFunction = (delta: number) => {}) {
		...
		public Init(StartFrom: IPosition, DateTime?: number): void {
			// this function will request toc.js file and then OnDone callback will be proceeded after toc.js contents parsed
		...
		private CloneTOCNodes(TOC: ITOC[]): ITOC[]{
			var NewTOC: ITOC[] = new Array();
		...
		private PatchToc(TOC: ITOC[], Range: IRange, Group: number): boolean {
			for (var I = 0; I < TOC.length; I++) {
				if (!TOC[I].t || TOC[I].t == '') {
					continue;
				}
		...
		private PatchTocRow(TOC: ITOC, Group: number): ITOC {
			if (!TOC.bookmarks) {
				TOC.bookmarks = {};
			}
		...
		public SearchForText(Text: string): ITOC[]{ return null }
		...
		public PageForward(): void {
			...
			if (this.CurStartPage !== undefined) { // Wow, we are on the pre-rendered page, things are quite simple!
				if (this.CurStartPage + this.NColumns < this.PagesPositionsCache.Length()) { // We know how many pages we have so we can check if the next one exists
					...
					this.OutOfBoundsFunction(this.CurStartPage + this.NColumns - this.PagesPositionsCache.LastPage());
					return;
				} else { // If cache is not yet ready - let's wait a bit.
					this.MoveTimeoutID = window.setTimeout(() => { this.PageForward() }, 50)
				}
			} else {
		...
		/**
			...
			*/
		public GoToXPath(XP: IXPath): void {
		...
		private GetElementXY(Node: IFB3Block): IDimensions {
		...
		public GetElementXYByPosition(Position: IPosition): IDimensions {
			var ResponcibleNode: IFB3Block = this.FB3DOM.GetElementByAddr(Position);
			return this.GetElementXY(ResponcibleNode);
		}

		/**
		 * This method checks if all the chunks are cached (cached meaning data is for all chunks are loaded and parsed)
		 * If not all data is cache - proceed some more data and check again after
		 * @param PageData
		 */
		private IdleGo(PageData?: IPageContainer): void {
...
//							alert('Execution time: ' + time);
		...
		public IdleOn(): void {
			...
			this.IdleTimeoutID = window.setInterval(() => { this.IdleGo() }, 100);
		}
		...
		public PatchRangeTo(Range: IRange): IRange {
        ...
        public SetPageViewTimer() {
            ...
            this.PageViewTimer = window.setInterval(handler, timeoutMSec);
        ...
        private PageReadTimersHandler() {
            ...
            this.PageReadCurrentTimer = window.setTimeout(handler, timeoutMSec /* HACK TODO 1 */);
		...
		private ActiveZonesForPage(Page: FB3ReaderPage.ReaderPage): IActiveZone[] {
			var xpid = '',
				activeElement = null;
